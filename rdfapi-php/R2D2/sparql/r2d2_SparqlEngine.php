...
*
* <BR><BR>History:<UL>
* <LI>01-09-2006               : First version of this class</LI>
*
* @version  V0.1
...
Class r2d2_SparqlEngine extends Object{
	...
	*
	* @param  DatasetMem       $dataset    the RDF Dataset
	...
	*/
	function queryModel($dataset,$query,$resultform = false){

		$this->log = new logging(TRUE);
		$this->query   = $query;
		...
		// get geht virtual graph und propertyBridges
		$R2D2Graph = $dataset->getDefaultGraph();
		$this->propertyBridges = $R2D2Graph->getPropertyBridges();


		// ------------------
		// search for relevant propertyBridges
		// ------------------
		$triples = array();
		$resultVars = $query->getResultPart();
		foreach($resultVars as $cur_var){			
			$triplePatterns = $cur_var->getTriplePattern();			
			if(!$triplePatterns) $triplePatterns = array();

			$triples[$cur_var->getID()] = $triplePatterns;
			
		}
		unset($cur_var);

		// find all possible propertyBridges for each triple pattern
		foreach($triples as $key => $GraphPattern){
			$i=0;			
			foreach($GraphPattern as $cur_triplePattern){
				$tripleQuery = $this->findPropertyBridges($cur_triplePattern);				
				
				$this->tripleSearch[$key][$i] = $tripleQuery;
				$i++;
			}		
		}				

		// ------------------
		// set the result iterator
		// ------------------

		// create a result Iterator for each triple pattern
		foreach($this->tripleSearch as $key => $GraphPattern){
			$i=0;
			foreach($GraphPattern as $cur_triplePattern){
				// create resultIterator for triple
				$this->tripleSearch[$key][$i]->setResultIterator();
				$i++;
			}
		}
		unset($GraphPattern);
		unset($cur_triplePattern);

		// ------------------
		// generate the SQL code over all patterns
		// ------------------
		$sql = $this->getSQL();

		reset($this->tripleSearch);
		// get the database connection
		$tmp = current($this->tripleSearch);
		$db = $tmp[0]->getDatabase();
		$this->triplesRes = new r2d2_TripleResult($db);

		// set the map for every SELECT var in the sql statement
		$this->createNewColumnNameMap();
		$this->triplesRes->setSQL($sql);
		$this->triplesRes->addTripleMaker($this->TripleMap);


		// ------------------
		// exec all sql statements and build triples
		// ------------------
		$resultTriples = array();
		// take every query, set it up and create triples

		if($this->log->debug("SQL query: ".$sql."\n<p>"));

		$this->triplesRes->execSQLQuery($sql);

		$resultTriples = $this->triplesRes->BuildTriples($resultVars[0]->triplePattern);

		// ------------------
		// set the search variables from each triple pattern
		// ------------------

		// select all search values (?x) from every triple pattern
		$ToSelect = $query->getResultVars();

		/*$i=0;
		foreach ($this->tripleSearch as $curTriple){
		$subject =  $this->tripleSearch[$i]->getSubject();
		$predicate =  $this->tripleSearch[$i]->getPredicate();
		$object  =  $this->tripleSearch[$i]->getObject();

		if (in_array($curTriple->subject->getNodeName(),$ToSelect))
		$this->tripleSearch[$i]->subject->setIsResultVar();
		if (in_array($curTriple->predicate->getNodeName(),$ToSelect))
		$this->tripleSearch[$i]->predicate->setIsResultVar();
		if (in_array($curTriple->object->getNodeName(),$ToSelect))
		$this->tripleSearch[$i]->object->setIsResultVar();

		if (tools::checkSparqlNodeType($subject))
		$this->tripleSearch[$i]->subject->NodeIsSet();
		if (tools::checkSparqlNodeType($predicate))
		$this->tripleSearch[$i]->predicate->NodeIsSet();
		if (tools::checkSparqlNodeType($object))
		$this->tripleSearch[$i]->object->NodeIsSet();
		...
		// ------------------
		// create all triples for each triple pattern
		// ------------------
		/*$result  = array(array());
		foreach($resultTriples as $curResults){  // run every SQL query
		$iTriplePattern = 0;
		foreach ($curResults as $curTriplePattern){
		foreach ($curTriplePattern as $row){   // take every row from first Triple Pattern
		foreach($row as $cur_triple){   // take all variables
		$triplePattern = $this->tripleSearch[$iTriplePattern];

		$position = count($result[$iTriplePattern]);

		if(!$triplePattern->subject->getIsSet()){
		$subject = $triplePattern->getSubject();
		$result[$iTriplePattern][$position][$subject->getNodeName()]	= $cur_triple->getSubject();
		}

		if(!$triplePattern->predicate->getIsSet()){
		$predicate = $triplePattern->getPredicate();
		$result[$iTriplePattern][$position][$predicate->getNodeName()]	= $cur_triple->getPredicate();
		}

		if(!$triplePattern->object->getIsSet()){
		$object = $triplePattern->getObject();
		$result[$iTriplePattern][$position][$object->getNodeName()]	= $cur_triple->getObject();
		}
		}
		}
		$iTriplePattern++;
		...
		unset($resultTriples);
		unset($curTriplePattern);
		unset($row);
		unset($position);*/

		// ------------------
		// combine all triple results and filter the variables set in resultSet
		// ------------------
		$resultSet = $resultTriples[0];
		for ($i=1; $i < count($resultTriples);$i++){
			$resultSet = $this->joinTuples($resultTriples[0],$resultTriples[$i]);
		}

		// ------------------
		// return the resultSet
		// ------------------
		$vartable[0]['patternResult'] = $resultSet;
	...
	function getSQL(){
		...
		$ToSelect = $this->query->getResultVars();

		// get all triple Patterns
		$resultVars = $this->query->getResultPart();
		foreach($resultVars as $cur_var){
			$triples = $cur_var->getTriplePattern();

		}

		$SQLQueries = array();
		
		foreach($this->tripleSearch as $ID => $GraphPattern){
			
			// combines every possible query from a triplePattern with each other
			$combiner = new r2d2_combiner($GraphPattern);

			// combine all possible queries
			$this->TripleMap = $combiner->combineTriples();  // combines all triple informations
			$this->SQLMap    = $combiner->combineSQL();      // combines all SQL informations to generate SQL


			// check resultForm (Select / Select distinct)
			$ResultForm = $this->query->getResultForm();
			if ( ($ResultForm !== "select distinct") && ($ResultForm !== "select"))
			return "SELECT FALSE";

			$sql = new r2d2_SQLGenerator();

			// set the distinct operator
			if ($ResultForm === "select distinct")
			$sql->setDistinct();
			
			// set options for Graph Pattern
			$options = $resultVars[$ID];			
			$sql->setUnion($options->getUnion());
			$sql->setOptional($options->getOptional());
			$sql->setContraint($options->getConstraint());
			

			for($iCounter =0; $iCounter<count($this->SQLMap);$iCounter++){ // run every possible query from query combiner

				$SQLInfo = $this->SQLMap[$iCounter]; // get current SQL informations
				$TripleInfo = $this->TripleMap[$iCounter]; // get current Triple informations

				$i=0;
				foreach($GraphPattern as $cur_Pattern){
					if($cur_Pattern->getTriple() === $TripleInfo[0]->getTriplePattern())
					break;
					$i++;
				}
				unset ($cur_Pattern);

				$subject   = $GraphPattern[$i]->getSubject();
				$predicate = $GraphPattern[$i]->getPredicate();
				$object    = $GraphPattern[$i]->getObject();

				$tableAlias = "t".($iCounter+1);   // a table alias

				if (tools::checkSparqlNodeType($subject))
				$subject->NodeIsSet();
				else{
					// find joint variables and new variables to be added
					for($j=0; $j<$iCounter;$j++){  //count($this->TripleMap)
						$OtherTriplePattern = $this->TripleMap[$j][0]->getTriplePattern();
						if (($subject->getNodeName() == $OtherTriplePattern->getSubject())
						&&($j != $i) && ( $TripleInfo[0]->getTriplePattern() !== $OtherTriplePattern) ){

							// get Column from current Pattern
							$xColumns = $TripleInfo[0]->getSubjectColumns();
							foreach($xColumns as $xColumn){
								$xcolumnName = $xColumn->getColumnName();
								$xtableName  = $xColumn->getTableName();

								// get Column from other Patterns
								$yColumns = $this->TripleMap[$j][0]->getSubjectColumns();
								foreach($yColumns as $yColumn){
									$ycolumnName = $yColumn->getColumnName();
									$ytableName  = $yColumn->getTableName();

									// join this statements
									if( ($xcolumnName === $ycolumnName) && ($xtableName === $ytableName)){
										$term = $tableAlias .".".$xcolumnName . "="."t".($j+1).".".$ycolumnName;
										$sql->addWHERE($term);
									}
								}
							}
				...
				$SQLtable = $SQLInfo->getSQLFrom();
				if($SQLtable){
					$x=0;
					foreach($SQLtable as $cur_table){
						// replace old Alias Map
						$pos = strpos($cur_table,' AS ');
						if($pos){							
							$alias = trim(substr($cur_table,$pos+3));
							$cur_table = substr($cur_table,0,$pos);	
							$key = array_search($alias,$SQLInfo->referedTables);						
							$oldAliasMap[$key] = $alias; 
						}
						
						//$tableMapping->addTriplePattern($cur_table,$tableAlias);
						if ($x==0){
							$sql->addFROM($cur_table." ".$tableAlias);
							$sql->addTableMap($iCounter,$cur_table,$tableAlias);
							$x++;
						}
						else{ // add join-table																			
							$altTableAlias = $tableAlias."j".$x;
							$sql->addFROM($cur_table." ".$altTableAlias);
							$sql->addTableMap($iCounter,$cur_table,$altTableAlias);
							$x++;
						}
					}
				}

				$SQLselect = $SQLInfo->getSQLSelect();
				if($SQLselect){
					$x=1;
					foreach($SQLselect as $cur_select){
						if(count($SQLselect) == 1)
						$cur_select = str_replace($SQLtable,$tableAlias,$cur_select);
						else{
							$pos = strpos($cur_select,'.');
							$table = substr($cur_select,0,$pos);

							$keyAlias = $sql->getTableMap($table,$iCounter);

							$cur_select = str_replace($SQLtable,$keyAlias,$cur_select);
							$x++;
						}
						$sql->addSELECT($cur_select);
					}
				}

				$SQLwhere = $SQLInfo->getSQLWhere();
				if($SQLwhere){
					foreach($SQLwhere as $cur_where){
						trim($cur_where);

						// search for first table
						$pos = strpos($cur_where,'.');
						if($pos){
							$table = substr($cur_where,0,$pos);
							if(in_array($table,$oldAliasMap))
								$table = $oldAliasMap[$table];
							
							$rest = substr($cur_where,$pos);
							$keyAlias = $sql->getTableMap($table,$iCounter);
							
							$cur_where = $keyAlias.$rest;
						}

						// search for "="
						$middle = strpos($cur_where,'=');
						if($middle){
							$pos = strpos($cur_where,'.',$middle);
							if($pos){
								$first = substr($cur_where,0,$middle);							
								$rest = substr($cur_where,$pos);
								$table = substr($cur_where,$middle+1,$pos-$middle-1);
								$key = array_search($table,$oldAliasMap);
								
								$keyAlias = $sql->getTableMap($table,$iCounter,$key);
								
								$cur_where = $first.'='.$keyAlias.$rest;								
							}
						}

						$sql->addWHERE($cur_where);
					}// SQLWhere
				}
			} // end iPatternCounter
			...
			// -------------------------------------------------------
			// check if SQL is "too long"
			// -------------------------------------------------------

			//if(count($sql->getSQLFROM())>10){
			//	$this->log->error("SPARQL query to complex!\nSQL query would take more than 10 referred tables");
			//	}
		...
		return r2d2_SQLGenerator::buildSQL($SQLQueries);
	 ...
	 * @return string
	 */
	function findPropertyBridges($triple){

		$s   = $triple->getSubject();
		$p   = $triple->getPredicate();
		$o   = $triple->getObject();

		$triplePattern = new r2d2_TripleGenerator($triple);

		$s = tools::checkSparqlNodeType($s);
		$p = tools::checkSparqlNodeType($p);
		$o = tools::checkSparqlNodeType($o);

		$it = $this->propertyBridges;
		for(reset($it); $bridge=current($it);next($it)):
			if(!$bridge->searchCorrectBridge($s,$p,$o,$triplePattern->getQueryContext())){
				continue;
			}
			$ID = $bridge->getID();
			$this->log->debug( "-------------------------------------------------------------<br>\n");
			...
			$tripleQuery = new TripleQuery($bridge, $s, $p, $o, $triple);
			$triplePattern->AddCombiner($tripleQuery);
	...
	function createNewColumnNameMap(){


		$ColumnNameNumberMap = array(array()); //[SQLQuery][TriplePattern][ColumnMap]

		$patternCounter = 0;
		$ColumnCounter = 1;
		foreach ($this->SQLMap as $cur_SQLMap){

			$ColumnMap = array();
			//$mapArray = $map->columnNameNumberMap;
			$mapArray = $cur_SQLMap->getColumnNameNumberMap();
			natsort($mapArray);  // sort Map-array

			foreach ($mapArray as $cur_mapElement=>$number){
				$ColumnMap[$ColumnCounter] = $cur_mapElement;

				$ColumnNameNumberMap[$patternCounter] = $ColumnMap;
				$ColumnCounter++;
		...
		$this->triplesRes->setColumnMap($ColumnNameNumberMap);
	}
	...
	* @return void
	*/
	function matchPattern(&$patternlist, $graphlist, &$graphPattern) {
	...
	* @return Array
	*/
	function findTuplesMatchingOnePattern($pattern, $graphlist){
		$var = null;
		$sub  = $pattern->getSubject();
		$pred = $pattern->getPredicate();
		$obj  = $pattern->getObject();

		if(is_string($sub)|| (is_a($sub,'BlankNode'))){
			if(is_string($sub))
			$var['sub'] = $sub;
			$sub = null;
		}
		if(is_string($pred)|| (is_a($pred,'BlankNode')) ){
			if(is_string($pred))
			$var['pred'] = $pred;
			$pred = null;
		}
		if(is_string($obj)|| (is_a($obj,'BlankNode'))){
			if(is_string($obj))
			$var['obj'] = $obj;
			$obj = null;
		}
			...
			foreach($graphlist['list'] as $key => $graphnode){
				...
				if($it->valid()){
					...
					while($it->valid()){
						if($graphnode == null){
							$element = $it->current();
							$element = $element->getStatement();
							$grname  = $it->current();
							$grname = $grname->getGraphname();
						}else{
							$element = $it->current();
							$grname  = $graphnode;
						}
	...
	* @return boolean
	*/
	function checkIntBindings($trip, $intBindings){
		switch($intBindings){
			case -1:
				return true;
				break;
	...
	*/
	function joinOptionalTuples(&$finalRes, &$res) {
	...
	* @return Array
	*/
	function _checkGraphs(&$pattern,$graphlist){
	...
	* @return Array
	*/
	function _buildIntBindings($var){
		$intBindings = -1;
		if(!$var)
		return $intBindings;
	...
	* @return Array
	*/
	function _buildResultSet($pattern,$resmodel){
	...
	* @return Array
	*/
	function selectVars($table,$vars){
		if($vars[0]=='*')
	...
	* @return Array
	*/
	function joinResults($patternlist){
	...
	*/
	function filterPatterns($patternlist,$outer){
		if($outer)
		$filter = 'outerFilter';
		else
		$filter = 'innerFilter';
	...
	* @return String
	*/
	function fillConstraintString($vars,$res,$constraint,$function){
		...
		if($function['isBlank']){
			foreach($function['isBlank'] as $var){
				if(isset($res[$var]) && $res[$var]!="" && (is_a($res[$var],'BlankNode')))
				$replacement = 'true';
				else
				$replacement = 'false';
		...
		if($function['isLiteral']){
			foreach($function['isLiteral'] as $var){
				if(isset($res[$var]) && $res[$var]!="" && (is_a($res[$var],'Literal'))  )
				$replacement = 'true';
				else
				$replacement = 'false';
		...
		if($function['isUri']){
			foreach($function['isUri'] as $var){
				if(isset($res[$var]) && $res[$var]!="" && (is_a($res[$var],'Resource')) && $res[$var]->getUri() && !(is_a($res[$var],'BlankNode')) )
				$replacement = 'true';
				else
				$replacement = 'false';
		...
		if($function['lang']){
			foreach($function['lang'] as $var){
				if(isset($res[$var]) && $res[$var]!="" && (is_a($res[$var],'Literal')) && $res[$var]->getLanguage() )
		...
		if($function['datatype']){
			foreach($function['datatype'] as $var){
				if(isset($res[$var]) && $res[$var]!="" && (is_a($res[$var],'Literal')) && $res[$var]->getDatatype() )
	...
	*/
	function buildVarmap($order, $vartable){
		$n= 0;
		$result = array();
		$num_var = array();
		...
		foreach($vartable as $k => $x){
			foreach($order as $value){
				...
				if($x[$value['val']]){
					...
					if(is_a($x[$value['val']],'Resource')){
						$pref = "1";
					}
					if(is_a($x[$value['val']],'BlankNode')){
						$pref = "0";
					}
	...
	*/
	function constructGraph($vartable,$constructPattern){
	...
	* @return MemModel
	*/
	function describeGraph($vartable){
	...
	*/
	function _determineType($var ,$resultGraph ){
		$type = null;
		// find in namedGraphs
		if(!(is_a($var,'Literal'))){
	...
	* @return void
	*/
	function _getAttributes($list,$resultGraph, $varvalue){
		if($list){
			foreach($list as $attribute){
				if(!(is_a($varvalue.'Literal'))){
	...
	*/
	function returnResult($vartable,$resultform = false){
		$result = "false";
	...
	*/
	function buildXmlResult($vartable){

		if(is_a($vartable,'NamedGraphMem') )
	...
	*/
	function _getBindingString($varname,$varvalue){
		$binding = '<binding name="'.$varname.'">';
		$value = '<unbound/>';

		if(is_a($varvalue,'BlankNode' )){
			$value = '<bnode>'.$varvalue->getLabel().'</bnode>';
		}elseif (is_a($varvalue,'Resource')){
			$value = '<uri>'.$varvalue->getUri().'</uri>';
		}elseif (is_a($varvalue,'Literal')){
