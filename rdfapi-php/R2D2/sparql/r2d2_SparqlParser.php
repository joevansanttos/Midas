...
* @package sparql
*/
Class r2d2_SparqlParser extends Object{
	...
	*/
	var $sops = array('regex','bound','isuri','isblank','isliteral','str','lang','datatype','langmatches');
	...
	*/
	function r2d2_SparqlParser(){
		$this->query          = new r2d2_Query();
	...
	*/
	function parse($queryString = false){
		if($queryString){
				...
				$e = new r2d2_SparqlParserException("Querystring is empty.",null,key($this->tokens));
	...
	*/
	function uncomment($queryString){
			...
			if($this->varCheck(current($this->tokens))|strtolower(current($this->tokens))=='*'){
				...
				if(strtolower(current($this->tokens))=='distinct'){
					...
					if($this->varCheck(current($this->tokens))|strtolower(current($this->tokens))=='*'){
						...
						echo SparqlParserException ("Variable or '*' expected.",null,key($this->tokens));
					}
				}
			}

			if(!current($this->tokens)){
				echo new r2d2_SparqlParserException("Unexpected end of File. ",null,key($this->tokens));
				break;
			}
		}
		prev($this->tokens);
	...
	function parseFrom(){
		...
		if(strtolower(current($this->tokens))!='named'){
			if($this->iriCheck(current($this->tokens))||$this->qnameCheck(current($this->tokens))){
				...
				echo new r2d2_SparqlParserException("Variable, Iri or qname expected in FROM ",null,key($this->tokens));
			}
			$this->query->addFrom(current($this->tokens));
		}else{
			$this->_fastForward();
			if($this->iriCheck(current($this->tokens))||$this->qnameCheck(current($this->tokens))){
				...
				echo new r2d2_SparqlParserException("Variable, Iri or qname expected in FROM NAMED ",null,key($this->tokens));
			}
		}
	}
	...
	*/
	function parseConstruct(){
		...
		if(current($this->tokens)=="{"){
			...
			echo new r2d2_SparqlParserException("Unable to parse CONSTRUCT part. '{' expected. ",null,key($this->tokens));
		}
		$this->parseWhere();
		$this->parseModifier();
	}
	...
	*/
	function parseWhere(){
		...
		if(current($this->tokens)=="{"){
			...
			echo new r2d2_SparqlParserException("Unable to parse WHERE part. '{' expected in Query. ",null,key($this->tokens));
		}
	}
	...
	*/
	function qnameCheck($token){
		...
		if(preg_match($pattern,$token,$hits)>0){
			...
			if($hits{1}=="_")
			return true;
			echo new r2d2_SparqlParserException("Unbound Prefix: <i>".$hits{1}."</i>",null,key($this->tokens));
		}else{
			$result = false;
		}
		return $result;
	...
	* @return void
	*/
	function parseGraphPattern($optional = false, $union = false, $graph = false,$constr = false, $external = false){
		$pattern = & $this->query->getNewPattern($constr);
		
		$i=0;
		foreach($this->query->getResultPart() as $curRes){
			if($curRes->getId() === $pattern->getId() )
				break;
			$i++;
		}
		$resultPart = & $this->query->getResultPart();
		$pattern = & $resultPart[$i];
	...
	* @return void
	*/
	function parseTriplePattern(&$pattern){
		$trp      = Array();
		$prev     = false;
		$prevPred = false;
		$cont     = true;
		...
		do{
			switch(strtolower(current($this->tokens))){
				...
				$trp[] = new r2d2_QueryTriple($sub,$pre,$obj);
				$this->_fastForward();
				break;
		...
		if(count($trp)>0){
			...
			//$this->query->resultPart[0] = $pattern;
		}
	}
	...
	* @return void
	*/
	function parseConstraint($pattern,$outer){
		$constraint = new r2d2_Constraint();
	...
	* @throws SparqlParserException
	*/
	function parseBrackettedExpression($constraint){
		$open = 1;
		$exp = "";
		$this->_fastForward();
		while($open !=0 & current($this->tokens)!= false){
			switch(current($this->tokens)){
				...
				break;
				case false:
				echo new r2d2_SparqlParserException ("Unexpected end of query.",null,key($this->tokens));
				default:
				$exp = $exp.current($this->tokens);
				break;
			}
	...
	* @throws SparqlParserException
	*/
	function parseExpression($constraint){
		$exp = "";
	...
	*/
	function parseGraph(){
		...
		if(!$this->varCheck($name)&!$this->iriCheck($name)&&!$this->qnameCheck($name)){
			$msg = $name;
			$msg = preg_replace('/</', '&lt;', $msg);
			echo new r2d2_SparqlParserException(" IRI or Var expected. ",null,key($this->tokens));
		}
		$this->_fastForward();
		...
		do{
			switch(strtolower(current($this->tokens))){
				...
				if(strtolower(current($this->tokens))=='by'){
					...
					echo new r2d2_SparqlParserException("'BY' expected.",null,key($this->tokens));
				}
				break;
				case "limit":
				$this->_fastForward();
		...
		& strtolower(current($this->tokens))!= 'offset'){
			switch (strtolower(current($this->tokens))){
				...
				if($this->varCheck(current($this->tokens))){
					...
					echo new r2d2_SparqlParserException("Variable expected in ORDER BY clause. ",null,key($this->tokens));
				}
				$this->_fastForward();
				if(current($this->tokens)!=')')
				echo new r2d2_SparqlParserException("missing ')' in ORDER BY clause.",null,key($this->tokens));
				$val['type'] = 'desc';
				$this->_fastForward();
				break;
				case "asc" :
	...
	*/
	function parseNode($node = false){
		...
		if($this->iriCheck($node)){
			...
			echo new r2d2_SparqlParserException($node." is neither a valid rdf- node nor a variable.",null,key($this->tokens));
		}
		return $node;
	}
	...
	* @return void
	*/
	function parseLiteral($node,$sep){
		do{
			next($this->tokens);
	...
	*/
	function parseCollection($trp){
		...
		while(current($this->tokens)!=")"){
			if($i>0)
			$trp[] = new r2d2_QueryTriple($this->parseNode($tmpLabel),new Resource("http://www.w3.org/1999/02/22-rdf-syntax-ns#rest"),$this->parseNode($tmpLabel = $this->query->getBlanknodeLabel()));
			$trp[] = new r2d2_QueryTriple($this->parseNode($tmpLabel),new Resource("http://www.w3.org/1999/02/22-rdf-syntax-ns#first"),$this->parseNode());
			$this->_fastForward();
			$i++;
		}
		$trp[] = new r2d2_QueryTriple($this->parseNode($tmpLabel),new Resource("http://www.w3.org/1999/02/22-rdf-syntax-ns#rest"),new Resource("http://www.w3.org/1999/02/22-rdf-syntax-ns#nil"));
		return $firstLabel;
	}
